...
def _ensure_bootstrap_admin(db) -> User:
...
def _bootstrap_admin_token() -> str:
    db = SessionLocal()
    try:
        admin = _ensure_bootstrap_admin(db)
        login_response = client.post(
            "/api/v1/auth/login",
            json={"username": admin.username, "password": "Admin@12345"},
        )
...
def _create_customer(db) -> Customer:
    ...
    customer = Customer(
        ...
        phone="9999999999",
        billing_address="Aerospace Park",
        shipping_address="Receiving Bay",
        is_active=True,
    )
    db.add(customer)
    db.commit()
...
def _create_production_operation(db, *, production_order_id: int, created_by: int) -> ProductionOperation:
    operation = ProductionOperation(
        ...
        operation_number=10,
        operation_name="Lifecycle Operation",
...
def _create_coc(db, *, production_order_id: int, issued_by: int) -> CertificateOfConformance:
    coc = CertificateOfConformance(
        ...
        remarks="Complete ERP lifecycle CoC",
    )
    db.add(coc)
    db.commit()
    db.refresh(coc)
...
def test_complete_erp_system():
    ...
    assert create_admin_response.status_code == 200, create_admin_response.text
    admin_user = create_admin_response.json()
    ...
    enquiry_response = client.post(
        ...
        json={
            ...
            "currency": "INR",
            "notes": "Complete ERP integration enquiry",
            "status": "draft",
        },
        headers=headers,
    )
    ...
    contract_review_response = client.post(
        ...
        json={
            ...
            "review_comments": "Approved for integration lifecycle",
        },
        headers=headers,
    )
    ...
    supplier_approval_response = client.post(
        ...
        json={
            "approved": True,
            "approval_remarks": "Approved for lifecycle flow",
    ...
    purchase_order_response = client.post(
        ...
        json={
            ...
            "remarks": "Integration PO",
            "quality_notes": "MTC required",
            "supplier_quality_requirements": "Full AS9100D requirements",
        },
        headers=headers,
    )
    ...
    issue_po_response = client.put(
        f"/api/v1/purchase/order/{purchase_order['id']}/status",
        json={"status": "issued"},
        headers=headers,
    )
    assert issue_po_response.status_code == 200, issue_po_response.text

    location_response = client.post(
        "/api/v1/stores/location",
        json={
            "location_code": _unique("LOC"),
            "location_name": "Integration Stores",
    ...
    rmir_response = client.post(
        f"/api/v1/stores/grn/{grn_item['id']}/inspect",
        json={
            "inspection_date": date.today().isoformat(),
            "inspection_status": "Accepted",
            "remarks": "RMIR accepted",
            "storage_location_id": location["id"],
        },
        headers=headers,
    )
    assert rmir_response.status_code == 200, rmir_response.text
    ...
    release_production_response = client.patch(
        f"/api/v1/production/order/{production_order['id']}/release",
        headers=headers,
    )
    assert release_production_response.status_code == 200, release_production_response.text

    db = SessionLocal()
    try:
        operation = _create_production_operation(
            db,
    ...
    start_operation_response = client.post(
        f"/api/v1/production/operation/{operation.id}/start",
        headers=headers,
    )
    assert start_operation_response.status_code == 200, start_operation_response.text

    inprocess_inspection_response = client.post(
        f"/api/v1/production/operation/{operation.id}/inspection",
        json={
            "inspection_result": "Pass",
            "remarks": "In-process passed",
    ...
    complete_operation_response = client.post(
        f"/api/v1/production/operation/{operation.id}/complete",
        headers=headers,
    )
    assert complete_operation_response.status_code == 200, complete_operation_response.text
    ...
    dispatch_order_response = client.post(
        ...
        json={
            ...
            "remarks": "Integration dispatch order",
        },
        headers=headers,
    )
    ...
    challan_response = client.post(
        ...
        json={
            ...
            "dispatch_order_id": dispatch_order["id"],
            "issue_date": date.today().isoformat(),
            "received_by": "Customer Stores",
            "remarks": "Integration challan",
        },
        headers=headers,
    )
