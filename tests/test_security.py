...
def _create_user_token(role_name: str, *, module_keys: list[str] | None = None) -> str:
    db = SessionLocal()
    try:
...
def _dispatch_create_payload(*, sales_order_id: int) -> dict:
    return {
        ...
        "remarks": "Security test dispatch creation",
    }
...
def test_sales_role_cannot_create_dispatch():
    _ensure_schema()
    ...
    assert "module access" in response.json()["detail"]